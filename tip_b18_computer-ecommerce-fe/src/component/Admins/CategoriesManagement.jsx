...
const CategoriesManagement = () => {
  ...
  const handleAdd = () => {
    axiosInstance.post('/categories/add', currentCategory)
      .then(() => {
  ...
  const handleEdit = () => {
    axiosInstance.put(`/categories/update/${currentCategory.id}`, currentCategory)
      .then(() => {
  ...
  return (
    <Container>
      ...
      <Button variant="primary" onClick={() => handleShowModal()}><FaPlus /> Thêm Danh mục</Button>
        ...
        <tbody style={{ textAlign: "center" }}>
          {currentCategories.map(category => (
            <tr key={category.id}>
              ...
              <td>{category.type}</td>
              <td>
                <Button variant="warning" onClick={() => handleShowModal(category)} className="me-2"><FaEdit /> Sửa</Button>
                <Button variant="danger" onClick={() => handleDelete(category.id)} className="me-2"><FaTrash /> Xóa</Button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Pagination className="justify-content-center">
        ...
        <Pagination.Next
          onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(filteredCategories.length / categoriesPerPage)))}
          disabled={currentPage === Math.ceil(filteredCategories.length / categoriesPerPage)}
        />
        <Pagination.Last
          onClick={() => setCurrentPage(Math.ceil(filteredCategories.length / categoriesPerPage))}
          disabled={currentPage === Math.ceil(filteredCategories.length / categoriesPerPage)}
        />
      </Pagination>
